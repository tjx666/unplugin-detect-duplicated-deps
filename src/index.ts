...
function parsePackageNameFromModuleId(id: string) {
    ...
    return {
        ...
        vite: {
                ...
                if (resolved) {
                    ...
                    if (packageInfo) {
                        ...
                        if (existedVersionsMap) {
                            ...
                            if (existedImporters) {
                                existedImporters.add(parsePackageNameFromModuleId(importer));
                            } else {
                                existedVersionsMap.set(
                                    version,
                                    new Set([parsePackageNameFromModuleId(importer)]),
                                );
                            }
                        } else {
                            const versionMap = new Map<string, Set<string>>([
                                [version, new Set([parsePackageNameFromModuleId(importer)])],
                            ]);
            ...
            buildEnd() {
                ...
                console.log();
                consola.warn(
                    `multiple versions of ${duplicatedPackages
                ...
                for (const duplicatedPackage of duplicatedPackages) {
                    ...
                    for (const version of sortedVersions) {
                        ...
                        console.warn(
                            `    - ${formattedVersion} imported by ${importers
